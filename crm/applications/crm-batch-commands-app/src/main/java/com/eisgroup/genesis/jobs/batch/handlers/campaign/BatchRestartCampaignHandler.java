 ...
 */
public class BatchRestartCampaignHandler extends BaseBatchCampaignHandler {

    public static final String NAME = "batchRestartCampaign";

    private static final String FILTER_STATE = "suspended";
    private static final String SUSPEND_TO = "suspendTo";
    private static final String RESTART_DATE = "restartDate";
    ...
    protected Observable<SubsequentCommand> execute() {
        return searchEntities()
                .map(v -> {
                    ...
                    jsonObject.addProperty(RESTART_DATE, LocalDate.now().toString());
                    return new SubsequentCommand(CrmCommands.RESTART_CAMPAIGN, () -> jsonObject, Variation.INVARIANT, v.getModelName());
                });
    }
    ...
    protected ModeledEntitySearchCriteria parseCriteria(Integer limit, Integer offset) {
        ...
        Collection<Matcher> searchMatchers = Arrays.asList(
                new SearchIndexQuery.FieldMatcher(SUSPEND_TO, SearchIndexQuery.FieldMatcher.Operand.lte, Collections.singletonList(LocalDate.now().atStartOfDay())),
