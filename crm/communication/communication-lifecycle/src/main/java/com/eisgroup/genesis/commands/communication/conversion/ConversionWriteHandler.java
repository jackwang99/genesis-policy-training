 ...
 */
package com.eisgroup.genesis.commands.communication.conversion;
 ...
 * Create/update Communication without some validations
 *
...
@Modifying
@Description("cowh002: Creates new Communication or updates existing.")
public class ConversionWriteHandler extends WriteHandler {
    ...
    @Override
    protected void copyLoadedEntityFields(Communication loaded, Communication current) {
        super.copyLoadedEntityFields(loaded, current);

        if(StringUtils.isBlank(current.getCommunicationId())){
            current.setCommunicationId(loaded.getCommunicationId());
        }
    }
    ...
    @Override
    protected void setupInitialDetails(Communication communication) {
        initAccessTrackInfo(communication);
    }    
