 ...
 * @author avoitau
 */
public class IndvContactsCountValidator implements ConstraintValidator<IndvContactsCount, GenesisCrmCommunicationInfo> {
    private int min;
    private int max;

    @Override
    public void initialize(IndvContactsCount parameters) {
        min = parameters.min();
        max = parameters.max();
        validateParameters();
    }
