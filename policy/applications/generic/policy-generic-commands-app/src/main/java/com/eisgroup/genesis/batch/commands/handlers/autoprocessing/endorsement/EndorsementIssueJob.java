...
/**
 * Batch job for issuing automatically created endorsements.
 *
 * @author ileanavets
 * @since 9.12
 */
public class EndorsementIssueJob extends AbstractStrategyJob {

    @Override
    public String getName() {
        return PolicyJobs.ENDORSEMENT_ISSUE_JOB;
    }
    ...
    protected ProcessedStepType getProcessedStepType() {
        return ProcessedStepType.ISSUE;
    }

    @Nonnull
    @Override
    protected Optional<CompositeRequest> getRequest(JobProcessingStrategy strategy, PolicySummary policySummary) {
        return Optional.of(new CompositeEntityKeyRequest(policySummary.getKey(), QuoteCommands.ISSUE_REQUEST,
                PolicyVariations.QUOTE.getName()));
    }
}
