...
package com.eisgroup.genesis.fleet.rating.integration;
...
/**
 * Rating configuration based on the calling remote Openl server for the Fleet Auto product
 *
 * @author DLiauchuk on 8/1/17.
 */
public class FleetAutoRatingConfiguration implements RatingConfiguration {

    private static final Logger LOGGER = LoggerFactory.getLogger(FleetAutoRatingConfiguration.class);

    private static final String RATING_SERVER_URL = "rating.server.url";
    private final String ratingServerUrl;

    public FleetAutoRatingConfiguration() {
        ratingServerUrl = System.getProperty(RATING_SERVER_URL);
        if (StringUtils.isEmpty(ratingServerUrl)) {
            LOGGER.error("Cannot load 'rating.properties' for Fleet Auto rating configuration");
        }
    }
