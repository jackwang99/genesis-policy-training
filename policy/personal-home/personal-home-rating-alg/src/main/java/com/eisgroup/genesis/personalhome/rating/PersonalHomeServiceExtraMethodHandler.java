...
/**
 * Interceptor that will handle the requests to the {@link PersonalHomeRatingService#rate(PersonalHomeRatingRequest)} method
 ...
 * @since 1.0
 */
public class PersonalHomeServiceExtraMethodHandler
        implements org.openl.rules.ruleservice.core.annotations.ServiceExtraMethodHandler<RatingResult> {
    
    private static final String PERSONAL_HOME_POLICY_CLASS = "org.openl.generated.beans.personalhome.Policy";
    private static final Logger LOGGER = LoggerFactory.getLogger(PersonalHomeServiceExtraMethodHandler.class);
    ...
    public RatingResult invoke(final Method interfaceMethod, Object serviceBean, Object... args) throws Exception {
        DestinationClassResolver destinationClassResolver = s -> {
            try {
                return interfaceMethod.getDeclaringClass().getClassLoader().loadClass(PERSONAL_HOME_POLICY_CLASS);
            }
        ...
        PersonalHomeRatingRequest request = (PersonalHomeRatingRequest) args[0];
        Object policy = mapper.map(request.getNode(), PERSONAL_HOME_POLICY_CLASS);
        ...
        return (DefaultRatingResult) serviceBean.getClass().getMethod("DeterminePolicyPremium",
                ...
                interfaceMethod.getDeclaringClass().getClassLoader().loadClass(PERSONAL_HOME_POLICY_CLASS))
    ...
    private Converter acceptTimeAsDate(Converter delegate) {
        return convertible -> {
            Assert.isInstanceOf(String.class, convertible);
