...
package com.eisgroup.genesis.personalauto.tfs.integration;
...
/**
 * TFS configuration based on the calling remote Openl server for the Personal Auto product
 *
 * @author Andrey Arsatyants
 */
public class PersonalAutoTfsConfiguration implements RatingConfiguration {

    private static final Logger LOGGER = LoggerFactory.getLogger(PersonalAutoTfsConfiguration.class);

    private static final String TFS_SERVER_URL = "rating.server.url";
    private final String tfsServerUrl;

    public PersonalAutoTfsConfiguration() {
        tfsServerUrl = System.getProperty(TFS_SERVER_URL);
        if (StringUtils.isEmpty(tfsServerUrl)) {
            LOGGER.error("Cannot load 'rating.properties' for Personal Auto tfs configuration");
        }
    }
