...
/**
 * TFS configuration based on the calling remote Openl server for the Garage Auto product
 *
 * @author Andrey Arsatyants
 */
public class GarageAutoTfsConfiguration implements RatingConfiguration {

    private static final Logger LOGGER = LoggerFactory.getLogger(GarageAutoTfsConfiguration.class);

    private static final String TFS_SERVER_URL = "rating.server.url";
    private final String tfsServerUrl;

    public GarageAutoTfsConfiguration() {
        tfsServerUrl = System.getProperty(TFS_SERVER_URL);
        if (StringUtils.isEmpty(tfsServerUrl)) {
            LOGGER.error("Cannot load 'rating.properties' for Garage Auto tfs configuration");
        }
    }
