...
package com.eisgroup.genesis.garage.rating.integration;
...
/**
 * Rating configuration based on the calling remote Openl server for the Garage Auto product
 *
 * @author DLiauchuk on 8/1/17.
 */
public class GarageAutoRatingConfiguration implements RatingConfiguration {

    private static final Logger LOGGER = LoggerFactory.getLogger(GarageAutoRatingConfiguration.class);

    private static final String RATING_SERVER_URL = "rating.server.url";
    private final String ratingServerUrl;

    public GarageAutoRatingConfiguration() {
        ratingServerUrl = System.getProperty(RATING_SERVER_URL);
        if (StringUtils.isEmpty(ratingServerUrl)) {
            LOGGER.error("Cannot load 'rating.properties' for Garage Auto rating configuration");
        }
    }
    ...
    public RatingParameters getRatingParameters() {
        ...
        builder.append(ratingServerUrl)
                .append("/com.eisgroup.genesis.garage.rating.RatingService");
